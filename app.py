...
# 🔑 [2번 방식 전면 연동] Secrets 상자에서 안전하게 환경 변수 호출
...
if df is not None:
    ...
    # 🎯 사용자가 명시한 열 구성 완벽 맵핑 완료
    # G열(6): 현재운전시간, H열(7): 정비매뉴얼, I열(8): 여분수량, J열(9): 최초장착일
    c_id = col_list[0]          # A열
    c_mach = col_list[1]        # B열
    c_name = col_list[2]        # C열
    c_mat = col_list[3]         # D열
    c_life_m = col_list[4]      # E열
    c_life_h = col_list[5]      # F열
    c_curr_h = col_list[6]      # G열
    
    idx_manual = 7              # H열 (인덱스 7)
    idx_stock = 8               # I열 (인덱스 8)
    idx_install = 9             # J열 (인덱스 9)
